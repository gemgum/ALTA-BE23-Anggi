		...
		if inputMenu == 1 {
			...
			for isLogin {
				...
				if inputMenu2 == 9 {
					...
					if err != nil {
						fmt.Println("error ketika update aktivitas")
						return
					}
					fmt.Println("berhasil update aktivitas")
					fmt.Println()
					...
					todo, err := tc.ShowTodo(data.ID)
					if err != nil {
						fmt.Println("error ketika menampilkan aktivitas")
						return
					}
					tc.ShowParseData(todo)
					// fmt.Println("List Acitivity", todo)
					fmt.Println()
				}
